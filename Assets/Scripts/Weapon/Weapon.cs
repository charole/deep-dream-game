...
{
  [SerializeField]
	private GameObject projectilePrefab; // 공격할 때 생성되는 발사체 프리랩
  [SerializeField]
	...
	private IEnumerator TryAttack()
	{
		while (true)
		{
			if (ammo.ConsumeAmmo(1)) // 발사 시 탄창 1발 소모
			{
					AttackByLevel();
					audioSource.Play();
			}
			yield return new WaitForSeconds(attackRate); // 공격 속도만큼 대기
		}
	}
	...
	private void AttackByLevel()
	{
		...
		case 3:
			Instantiate(projectilePrefab, transform.position, Quaternion.identity);
			cloneProjectile = Instantiate(projectilePrefab, transform.position, Quaternion.identity); // 발사체 생성 (왼쪽 대각선)
			cloneProjectile.GetComponent<Movement2D>().MoveTo(new Vector3(-0.2f, 1, 0));
			cloneProjectile = Instantiate(projectilePrefab, transform.position, Quaternion.identity); // 발사체 생성 (오른쪽 대각선)
			cloneProjectile.GetComponent<Movement2D>().MoveTo(new Vector3(0.2f, 1, 0));
			break;
		}
	}
